function M=ODE_simulation_lipoGlu(P, skipcorr)
    ...
    if isfield(P, 'Trec')
        %recovery from Zi
        M_sol=BM_ODEsolution_lipoGlu(P,0,P.Trec,M_sol);
    end
    
    %Saturation sequence
    if P.pulsed==0
        M_sol=BM_ODEsolution_lipoGlu(P,P.CALC.w1,P.tp,M_sol);
        M.sol=M_sol;
    ...
    elseif strcmp(P.shape,'block')
        for i=1:P.n
            ...
            M_sol=BM_ODEsolution_lipoGlu(P,0,P.td,M_sol);
        end
        ...
        for n=1:P.n %loop on each pulse
            %saturation pulse
            for i=1:steps
               M_sol=BM_ODEsolution_lipoGlu(P,gamma_2pi*B1(i),time_interval,M_sol);
            end
            %relaxation
            M_sol=BM_ODEsolution_lipoGlu(P,0,P.td,M_sol);
        end
    ...
    if P.play_readout
        ...
        M_sol=BM_ODEsolution_lipoGlu(P,0,P.TEeff,M_sol); %effective echo time relaxation
    end
    ...
    if strcmp(P.Offsets_order,'successive')
        ...
        for k=1:numel(P.xZspec)
            ...
            Mi_tmp=calculate_ODE_sol_lipoGlu(P, Mi_tmp, P.xZspec(k));
            Mi_calc(:,k)=Mi_tmp;
        end
        M_sol=Mi_calc;
    end
   ...
   if isfield(P,'normalized')
        M0=calculate_ODE_sol_lipoGlu(P, P.Zi*init_magnetisation_vector(P), P.normalized);
        M.M0=M0;
    ...
    if isrow(M.zspec)
        ...
        if P.WaterResCorr==1
            ...
            Ztmp=[ODE_simulation_lipoGlu(Ptmp).zspec;0];
