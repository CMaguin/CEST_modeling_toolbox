function M=numeric_simulation(P)
    ...
    %recovery from Zi
    M_sol=BM_solution(P,0,P.Trec,M_sol);
    ...
    if strcmp(P.Offsets_order,'successive')
        ...
        for k=1:numel(P.xZspec)
            ...
            Mi_tmp=calculate_numeric_sol(P, Mi_tmp, P.xZspec(k));
            Mi_calc(:,k)=Mi_tmp;
        end
        M_sol=Mi_calc;
    end
   ...
   if isfield(P,'normalized')
        M0=calculate_numeric_sol(P, P.Zi*init_magnetisation_vector(P), P.normalized);
        M.M0=M0;
    ...
    if isfield(P,'WaterResCorr')
        if P.WaterResCorr==1
            ...
            Ztmp=[numeric_simulation(Ptmp).zspec;0];
