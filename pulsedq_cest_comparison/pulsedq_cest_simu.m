function Msim=pulsedq_cest_simu(P)
...
PMEX = readSimulationParameters(param_filename);

PMEX=update_pulsedqcest(PMEX,P);

pulseqcestmex('init',PMEX,seq_filename)
tic
M = pulseqcestmex('run'); 
toc
M_z=M((1+P.n_cest_pools+P.MT)*2+1,:);
