function [Results,Fit, Pnew]=fit_data_lipoGlu_ODE(P,Z,OPTIM,options)
...
if OPTIM.multivar
    f = @(x,xdata) compute_ODE_multivar_lipoGlu(P, OPTIM, x);
end
else
    f = @(x,xdata) compute_ODE_lipoGlu(P, OPTIM, x);
end
...
try
    ...
    if isfield(OPTIM, 'multivar')
    if OPTIM.multivar
        f = @(x,xdata) compute_analytical_multivar_lipoGlu(P, OPTIM, x,densexxx);
    end
    else
        f = @(x,xdata) compute_analytical_lipoGlu(P, OPTIM, x,densexxx);
    end
